...
export function normalizePhone(rawPhone: unknown): string {
  ...
  if (!/^\d+$/.test(trimmed)) {
    throw new PhoneValidationError(
      'Phone number must contain exactly 10 digits without letters or symbols.'
    );
  }

  if (trimmed.length !== 10) {
    throw new PhoneValidationError(
      `Phone number must be exactly 10 digits (received ${trimmed.length}).`
    );
  }

  if (!VALID_INDIAN_MOBILE.test(trimmed)) {
    throw new PhoneValidationError(
      'Phone number must start with 6, 7, 8, or 9 for a valid Indian mobile number.'
    );
  }
 ...
 */
export function isValidIndianPhone(rawPhone: unknown): boolean {
  try {
