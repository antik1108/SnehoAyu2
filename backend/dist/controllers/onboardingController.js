...
import { validateMotherProfileInput, validateBabyProfileInput, validateHospitalCodeInput, validateCompleteOnboardingInput, } from '../validators/onboardingValidator.js';
import { formatParticipantCode, getGroupPrefix, extractSequenceFromParticipantCode, } from '../utils/participantCode.js';
...
function getMotherUserId(req) {
    ...
    if (req.user.role !== 'mother') {
        throw createError(403, 'MOTHER_ROLE_REQUIRED', 'Only authenticated mothers can access onboarding profile APIs.');
    }
    return req.user.id;
}
const API_TO_DB_AGE_RANGE = {
    below_18: 'below_18',
    '18_25': '18-25',
    '26_30': '26-30',
    '31_35': '31-35',
...
export async function saveMotherProfile(req, res, next) {
    try {
        ...
        if (!validation.valid || !validation.data) {
            next(validationError('Some profile fields are invalid.', validation.errors));
            return;
        }
...
export async function linkHospital(req, res, next) {
    try {
        const userId = getMotherUserId(req);
        ...
        if (!motherProfile) {
            next(createError(409, 'MOTHER_PROFILE_REQUIRED', 'Complete the mother profile before linking a hospital.'));
            return;
        }
        ...
        const alreadyLinked = motherProfile.hospitalId === hospital.id && req.user?.hospitalId === hospital.id;
        if (alreadyLinked) {
            res.status(200).json({
                success: true,
        ...
        if (motherProfile.participantCode !== null) {
            next(createError(409, 'HOSPITAL_LINK_LOCKED', 'The hospital cannot be changed after the participant code has been generated.'));
            return;
        }
...
export async function getOrCreateParticipantCode(req, res, next) {
    try {
        const userId = getMotherUserId(req);
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                result = await prisma.$transaction(async (tx) => {
                    ...
                    if (!motherProfile) {
                        throw createError(409, 'MOTHER_PROFILE_REQUIRED', 'Complete the mother profile before generating the participant code.');
                    }
                    ...
                    if (!motherProfile.babyProfile) {
                        throw createError(409, 'BABY_PROFILE_REQUIRED', 'Complete the baby profile before generating the participant code.');
                    }
                    // 5. Verify hospital link
                    if (!motherProfile.hospitalId || !user?.hospitalId) {
                        throw createError(409, 'HOSPITAL_LINK_REQUIRED', 'Link a hospital before generating the participant code.');
                    }
                    if (motherProfile.hospitalId !== user.hospitalId) {
                        throw createError(409, 'HOSPITAL_LINK_MISMATCH', 'User and profile hospital links do not match.');
                    }
                    ...
                    if (!hospital.isActive) {
                        throw createError(409, 'HOSPITAL_ENROLLMENT_CLOSED', 'Enrolment at this hospital is currently closed.');
                    }
                    ...
                    if (studyGroup === null) {
                        ...
                        if (process.env['NODE_ENV'] !== 'production' &&
                            ...
                            console.warn(`[DevWarning] Assigning studyGroup randomly for user ${userId} in development.`);
                            ...
                            throw createError(409, 'STUDY_GROUP_REQUIRED', 'The study group must be assigned by the researcher before generating a participant code.');
                        }
                    }
                    ...
                    const participantCode = formatParticipantCode(updatedHospital.code, studyGroup, sequence);
                    ...
                    if (updateResult.count === 0) {
                        throw createError(409, 'PARTICIPANT_CODE_CONFLICT', 'Participant code already assigned.');
                    }
                    return {
                        participantCode,
        ...
        if (!result) {
            throw createError(409, 'PARTICIPANT_CODE_CONFLICT', 'Could not generate participant code due to concurrent updates.');
        }
...
export async function completeOnboarding(req, res, next) {
    try {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                result = await prisma.$transaction(async (tx) => {
                    ...
                    if (!motherProfile) {
                        throw createError(409, 'MOTHER_PROFILE_REQUIRED', 'Complete the mother profile before finalizing.');
                    }
                    ...
                    if (isAlreadyCompleted) {
                        ...
                        if (dischargeDate) {
                            ...
                            for (const config of configs) {
                                const exists = motherProfile.followUpSchedules.some((s) => s.timePoint === config.timePoint);
                                if (!exists) {
                            ...
                            if (schedulesToCreate.length > 0) {
                                ...
                                console.warn(`[Warning] Repaired ${schedulesToCreate.length} missing schedules for completed participant ${motherProfile.participantCode || motherProfile.id}`);
                            }
                        }
                        ...
                        finalSchedules.sort((a, b) => (timePointOrder[a.timePoint] ?? 99) - (timePointOrder[b.timePoint] ?? 99));
                        return {
                    ...
                    if (!motherProfile.babyProfile) {
                        ...
                        for (const field of requiredBabyFields) {
                            const val = motherProfile.babyProfile[field];
                            if (val === null || val === undefined || val === '') {
                    ...
                    if (details.length > 0) {
                        throw createError(409, 'ONBOARDING_INCOMPLETE', 'Complete all required onboarding steps before finalizing.', details);
                    }
                    ...
                    if (parts.length !== 3)
                        consistent = false;
                    if (parts[0] !== hospital.code)
                        consistent = false;
                    if (studyGroup === 'study' && parts[1] !== 'S')
                        consistent = false;
                    if (studyGroup === 'control' && parts[1] !== 'C')
                        consistent = false;
                    ...
                    if (duplicate)
                        consistent = false;
                    if (!consistent) {
                        throw createError(409, 'PARTICIPANT_CODE_INCONSISTENT', 'Participant code is inconsistent with the stored hospital and study group.');
                    }
        ...
        if (!result) {
            throw createError(409, 'FOLLOW_UP_SCHEDULE_CONFLICT', 'Could not generate follow-up schedules due to concurrent updates.');
        }
