...
export async function register(req, res, next) {
    try {
        ...
        if (!validation.valid || !validation.data) {
            next(createError(400, 'VALIDATION_ERROR', validation.errors.map((e) => e.message).join(' ')));
            return;
        }
        const result = await registerUser(validation.data.phone, validation.data.password);
        res.status(201).json({
            success: true,
...
export async function login(req, res, next) {
    try {
        ...
        const result = await loginUser(validation.data.phone, validation.data.password);
        res.status(200).json({
            success: true,
...
export async function createPin(req, res, next) {
    try {
        ...
        const result = await createOrUpdatePin(getAuthenticatedUserId(req), validation.data.pin, validation.data.currentPassword);
        res.status(200).json({
            success: true,
...
export async function changePin(req, res, next) {
    try {
        ...
        const result = await changeUserPin(getAuthenticatedUserId(req), validation.data.currentPassword, validation.data.newPin);
        res.status(200).json({
            success: true,
...
// ---------------------------------------------------------------------------
export async function removePin(req, res, next) {
    try {
        const body = req.body;
        ...
        const result = await removeUserPin(getAuthenticatedUserId(req), validation.data.currentPassword);
        res.status(200).json({
            success: true,
