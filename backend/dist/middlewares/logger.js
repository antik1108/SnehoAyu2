...
// Log helpers
// ---------------------------------------------------------------------------
function formatDev(method, url, status, durationMs, contentLength) {
    const sc = statusColour(status);
    const length = contentLength ? ` ${colour.dim}${contentLength}b${colour.reset}` : '';
    return (`${colour.dim}[${new Date().toISOString()}]${colour.reset} ` +
...
function logProd(method, url, status, durationMs, contentLength, reqId) {
    const entry = {
