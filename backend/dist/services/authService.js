...
const DUMMY_PASSWORD_HASH = '$2b$12$oXG9rX4h9d39H0SAbc6xCe/51Kdy4/4tpOFoFFYaJdHng2lHEzS0S';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
...
function validatePinStrength(pin, phone) {
    ...
    if (repeated || ascending || descending) {
        throw createError(400, 'WEAK_PIN', 'Choose a less predictable PIN.');
    }
    if (barePhone.includes(pin)) {
        throw createError(400, 'WEAK_PIN', 'PIN must not contain part of your phone number.');
    }
}
async function verifyCurrentPassword(password, passwordHash) {
    const matches = await bcrypt.compare(password, passwordHash);
    if (!matches) {
        throw createError(401, 'INVALID_CREDENTIALS', 'Invalid current password.');
    }
}
 ...
 */
export async function registerUser(rawPhone, password) {
    // 1. Normalize
    const phone = normalizePhone(rawPhone);
    // 2. Duplicate check
    const existing = await prisma.user.findUnique({ where: { phone } });
    if (existing) {
        throw createError(409, 'PHONE_ALREADY_REGISTERED', 'An account with this phone number already exists.');
    }
 ...
 */
export async function loginUser(rawPhone, password) {
    ...
    if (user.passwordLockedUntil && user.passwordLockedUntil > new Date()) {
        ...
        throw createError(423, 'ACCOUNT_LOCKED', `Account is temporarily locked due to too many failed attempts. ` +
...
// PIN setup / mutation
// ---------------------------------------------------------------------------
export async function createOrUpdatePin(userId, pin, currentPassword) {
    const user = await findActiveUserById(userId);
    validatePinStrength(pin, user.phone);
    if (user.pinHash) {
        if (!currentPassword) {
            throw createError(400, 'CURRENT_PASSWORD_REQUIRED', 'Current password is required to update an existing PIN.');
        }
...
export async function changePin(userId, currentPassword, newPin) {
    const user = await findActiveUserById(userId);
    await verifyCurrentPassword(currentPassword, user.passwordHash);
...
export async function removePin(userId, currentPassword) {
    const user = await findActiveUserById(userId);
    await verifyCurrentPassword(currentPassword, user.passwordHash);
    await prisma.user.update({
        where: { id: user.id },
...
// PIN login
// ---------------------------------------------------------------------------
export async function loginWithPin(rawPhone, pin) {
    ...
    if (user.pinLockedUntil && user.pinLockedUntil > new Date()) {
        ...
        const remainingMinutes = Math.ceil(remainingMs / 60_000);
        throw createError(423, 'PIN_LOCKED', `PIN login is temporarily locked. Try again in ${remainingMinutes} minute${remainingMinutes === 1 ? '' : 's'}.`);
    }
